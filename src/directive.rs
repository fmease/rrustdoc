//! The parser of `ui_test` directives.

use crate::{
    builder::{CrateNameRef, Edition},
    parser,
    utility::default,
};
...
pub(crate) struct InstantiatedDirectives<'src> {
    pub(crate) dependencies: Vec<Dependency<'src>>,
...
impl<'src> InstantiatedDirectives<'src> {
    fn adjoin(&mut self, directive: DirectiveKind<'src>) {
        match directive {
            DirectiveKind::AuxBuild { path } => self.dependencies.push(Dependency::Crate { path }),
            DirectiveKind::AuxCrate { name, path } => self
                .dependencies
                .push(Dependency::NamedCrate { name, path }),
            ...
            // However, that's just how it is, they are treated verbatim by `ui_test`, so we do the same.
...
impl<'src> DirectiveParser<'src> {
    ...
    fn execute(mut self) -> Result<Directive<'src>, Error<'src>> {
        ...
        let revision = if self.consume(|char| char == '[') {
            // FIXME: How does `ui_test` deal with empty revision conditions (`//@[] ...`)?
        ...
        let kind = match directive {
            ...
            // `ui_test` doesn't support extern options (e.g., `priv`, `noprelude`, `nounused`, `force`)
            // and the time of writing. Therefore, we don't need to deal with them here either.
            "aux-crate" => {
                self.parse_separator(Padding::Yes)
                    .map_err(|error| error.context(context))?; // FIXME: audit AllowPadding
                ...
                let name = self.take_while(|char| !char.is_ascii_whitespace());
            ...
            // `ui_test` only supports a single environment variable per directive.
            "rustc-env" => {
                self.parse_separator(Padding::No)
                    .map_err(|error| error.context(context))?;
                let line = self.take_remaining_line();

                // FIXME: How does `ui_test` handle the edge cases here?
...
impl Report<'_> {
    fn publish(self) {
        if !self.unknowns.is_empty() {
            // FIXME
            eprintln!("[[ERROR]] UnknownDirectives({})", self.unknowns.join(", "));
        }

        for error in self.errors {
            eprintln!("[[ERROR]] {error:#?}"); // FIXME: temporary
        }
    }
}
