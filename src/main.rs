...
use builder::{BuildMode, CrateNameBuf, CrateNameCow, Edition};
...
fn main() -> error::Result {
    ...
    // FIXME: eagerly lower `-f`s to `--cfg`s here, so we properly support them in `ui_test`+command

    let build_mode = match (cross_crate, ui_test) {
        ...
        (crate_name, crate_type) => {
            let (crate_name, crate_type): (Option<CrateNameCow<'_>>, _) = match build_mode {
                BuildMode::Default | BuildMode::CrossCrate => {
                    ...
                    let attributes = Attributes::parse(&source, &build_flags.cfgs, edition);
