...
impl<'src> Attributes<'src> {
    pub(crate) fn parse(source: &'src str, cfgs: &[String], edition: Edition) -> Self {
        let attributes = AttributeParser::new(source, edition).execute();
        Self::lower(attributes, cfgs, source)
    }
