...
use crate::builder::{CrateNameBuf, CrateType, Edition, LintLevel};
use clap::{
...
// FIXME: Add `--color=<always|never|auto>`.
#[derive(Parser)]
#[command(about)]
pub(crate) struct Arguments {
    ...
    /// Enable `ui_test` directives: `//@`.
    #[arg(short = 'T', long, conflicts_with("cross_crate"))]
    pub(crate) ui_test: bool,
    /// Enable XPath / JsonPath queries (depending on `--json`). Implies `--ui-test`.
    #[arg(short = 'Q', long, conflicts_with("cross_crate"))]
